...
\begin{document}
	...
	Since the MIMO radar and the IBFD MU-MIMO communications system are overlaid in time domain, the communications Rxs experience interference due to the radar transmissions and vice versa. Following \cite{biswas2018fdqos,MCMIMO_RadComm} and the narrowband assumption, we model the CIRs for the paths: BS$\rightarrow$radar Rx $n\rr$, UL UE$i$ $\rightarrow$radar Rx $n\rr$, radar Tx $m\rr$ $\rightarrow$ BS, radar Tx $m\rr\rightarrow$ DL UE $j$ as \par\noindent\small
	\begin{flalign}
		\mathbf{g}_{\mathrm{Bm},n\rr}\paren{t,\tau}=&h_{\mathrm{Bm},n\rr}\mathbf{a}_{\mathrm{T}}^\dagger\paren{\theta_{\mathrm{B},n\rr}}e^{j2\pi f_{\mathrm{Bm},n\rr}t}\delta\paren{\tau-\tau_{\mathrm{Bm},n\rr}}\nonumber,
	\end{flalign}
	BS $\rightarrow$ target $\rightarrow$ radar Rx $n\rr$
	...
	We suppose that the moving target remains in the illumination region of the BS during the CPI. Because of the smaller transmission powers and antenna aperture of the UL UEs, we model that the UL signals arrive at the radar $n\rr$ via only the direct path because of the smaller antenna aperture of UL UEs while the DL signals arrive at the radar Rx $n\rr$ through a direct path and a target reflection path. The CIRs for the paths: $m\rr$ radar Tx $\rightarrow$ BS, $m\rr$ radar Tx $\rightarrow$ target $\rightarrow$ BS, and $m\rr$ radar Tx $\rightarrow$ UL UE $i$ are respectively modeled as 
	\par\noindent\small
	\begin{flalign}
	...
	\begin{flalign}
		...
		\approx&\sum_{m\rr=1}^{M\rr}\sum_{\kappa=1}^{\mathit{K}}\alpha_{m\rr\mathrm{B}}\mathbf{a}_\mathrm{R}\paren{\theta_{\target,\mathrm{B}}}e^{j2\pi f_{m\rr\mathrm{B}}kT_{\mathrm{r}}}a_{m_\mathrm{r},k}\nonumber\\
	...
	\par\noindent\small
	\begin{flalign}
		y_{\mathrm{r,B}}\bracket{n}=&\sum_{m\rr=1}^{M\rr}\widetilde{y}_{\mathrm{r,B}}\bracket{n}\ast \phi^\dagger_{m\rr}\bracket{-\paren{n+G}}e^{-j2\pi f_{m\rr,\B}kT\rr}\label{eq: radar_BS_1}
	\end{flalign}\normalsize
	\par\noindent\small
	\begin{flalign}
	...
	\end{flalign}\normalsize
	where \eqref{eq: radar_BS_1} peaks at $n=\paren{k-1}+n_{\mathrm{rB}}$ while \eqref{eq: radar_DL_1} peaks at $n=\paren{k-1}+n_{\mathrm{r,d}}$. Therefore, one can show that the radar signals interfere with the $\ith{n_{\mathrm{rmB}}}$ and $\ith{n_{\mathrm{rtB}}}$ symbol periods of the $\ith{k}$ UL frame at the BS Rx as well as with the $\ith{n_{\mathrm{r,d}}}$ symbol period of the $\ith{k}$ DL frame at the DL UE $j$, respectively, as
	\par\noindent\small
	\begin{flalign}
